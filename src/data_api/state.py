...
def clear_mutable(state):
    rasps, rooms = state.rasps, state.rooms
    initial_constraints = state.initial_constraints
    time_structure = state.time_structure

    state.grades                         = grade_tool.init_grades(rasps, rooms)
    state.timetable                      = tabl_api.get_empty_timetable(rasps)
    state.mutable_constraints            = cons_api.get_mutable_constraints(initial_constraints)
    state.rasp_rrules, state.rrule_space = time_api.init_rrule_objects(rasps, time_structure)
