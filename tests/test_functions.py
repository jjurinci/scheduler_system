...
def date_conversion_test(all_possible_dates):
    ...
    for date in all_possible_dates:
        week, day, hour = time_api.date_to_index(date, time_structure)
        calculated_date  = time_api.index_to_date(week, day, hour, time_structure)
...
def all_dtstart_weekdays_test(all_possible_dates):
    ...
    for date in all_possible_dates:
        five_dates = time_api.all_dtstart_weekdays(date,5)
        expected_weekday = 0
